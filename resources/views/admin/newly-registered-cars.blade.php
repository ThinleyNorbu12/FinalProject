        ...
        <div class="row g-3">
            ...
            <div class="col-md-2">
                <div class="d-flex gap-2">
                    <button type="button" id="clear-search" class="btn btn-outline-secondary clear-btn {{ request('search') ? '' : 'd-none' }}">
            ...
            <div class="card-body">
                <div class="empty-message text-center py-5">
                    ...
                    @if(request('search'))
                        ...
                        <p class="mb-3">No car registration requests match your search criteria: "<strong>{{ request('search') }}</strong>"</p>
                        <a href="{{ route('car-admin.new-registration-cars') }}" class="btn btn-outline-primary">
                                ...
                                <tr>
                                    <td>
                                        <div class="registration-date">
                                            <div class="fw-medium">{{ $car->created_at->format('M d, Y') }}</div>
                                            <small class="text-muted">{{ $car->created_at->format('h:i A') }}</small>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="fw-medium">{{ $car->maker }}</span>
                                    </td>
                                    <td>{{ $car->model }}</td>
                                    <td>
                                    ...
                                    <td>
                                        @if(strtolower($car->status) === 'pending')
                                            <span class="badge bg-warning text-dark">
                                                <i class="fas fa-clock me-1"></i>
                                                Pending
                                            </span>
                                        @elseif(strtolower($car->status) === 'approved')
                                    ...
                                    <td>
                                        @if($car->car_image)
                                            <img src="{{ asset($car->car_image) }}" alt="Car Image" class="img-thumbnail" style="width: 60px; height: 45px; object-fit: cover;">
                                        @else
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function performLiveSearch(query) {
            fetch(`{{ route('car-admin.live-search') }}?search=${encodeURIComponent(query)}`, {
                method: 'GET',
                headers: {
        ...
        function renderSearchResults(cars) {
            ...
            cars.forEach(car => {
                ...
                let carImage = '';
                if (car.car_image) {
                    carImage = `<img src="{{ asset('') }}${car.car_image}" alt="Car Image" class="img-thumbnail" style="width: 60px; height: 45px; object-fit: cover;">`;
                } else {
