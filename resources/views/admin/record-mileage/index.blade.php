            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <a href="{{ route('car-admin.record-mileage.create') }}" class="btn btn-primary">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <form action="{{ route('car-admin.record-mileage.search') }}" method="GET" class="d-flex">
                                ...
                                @if(request('search'))
                                    <a href="{{ route('car-admin.record-mileage') }}" class="btn btn-outline-secondary ml-2">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @foreach($cars as $car)
                                        <tr>
                                            <td>
                                                <strong class="text-primary">{{ $car->registration_no }}</strong>
                                            </td>
                                            <td>
                                                <div>
                                                    <strong>{{ $car->maker }} {{ $car->model }}</strong>
                                                </div>
                                            </td>
                                            <td>
                                                @if($car->mileage)
                                                    <span class="badge badge-info">{{ number_format($car->mileage) }} km</span>
                                                @else
                                                    <span class="text-muted">Not recorded</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if($car->start_mileage)
                                                    <span class="badge badge-success">{{ number_format($car->start_mileage) }} km</span>
                                                @else
                                            ...
                                            <td>
                                                @if($car->end_mileage)
                                                    <span class="badge badge-warning">{{ number_format($car->end_mileage) }} km</span>
                                                @else
                                                    <span class="text-muted">Not set</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if($car->start_mileage && $car->end_mileage)
                                                    <span class="badge badge-secondary">
                                                        {{ number_format($car->end_mileage - $car->start_mileage) }} km
                                                    </span>
                                                @elseif($car->start_mileage && $car->mileage)
                                                    <span class="badge badge-info">
                                                        {{ number_format($car->mileage - $car->start_mileage) }} km
                                                    </span>
                                                @else
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <a href="{{ route('car-admin.record-mileage.edit', $car->id) }}" 
                    ...
                    @else
                        <div class="text-center py-5">
                            ...
                            @if(request('search'))
                                ...
                                <a href="{{ route('car-admin.record-mileage') }}" class="btn btn-outline-primary">
                                    View All Cars
                                </a>
                            @else
