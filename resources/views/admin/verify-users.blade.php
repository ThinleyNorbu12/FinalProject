...
@push('styles')
     <link rel="stylesheet" href="{{ asset('assets/css/admin/verifyuser.css') }}">
@endpush
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body p-0 p-md-3">
                ...
                <table class="table table-bordered table-hover mb-0" id="users-table">
                    ...
                    <tbody>
                        ...
                        <tr class="status-row {{ $customer->drivingLicense ? strtolower($customer->drivingLicense->status) : 'incomplete' }}">
                            ...
                            <td class="text-nowrap">{{ $customer->cid_no }}</td>
                            <td class="text-nowrap">{{ $customer->drivingLicense ? $customer->drivingLicense->license_no : 'Not submitted' }}</td>
                            <td>
                            ...
                            <td class="text-nowrap">{{ \Carbon\Carbon::parse($customer->created_at)->format('d M Y') }}</td>
            ...
            <div class="mobile-cards d-block d-md-none">
                @forelse($customers as $customer)
                <div class="mobile-card status-{{ $customer->drivingLicense ? strtolower($customer->drivingLicense->status) : 'incomplete' }}">
                    <div class="mobile-card-header">
                        <div class="flex-grow-1">
                            <div class="mobile-card-title">{{ $customer->name }}</div>
                            <div class="mobile-card-subtitle">ID: {{ $customer->id }}</div>
                        </div>
                        <div>
                            @if(!$customer->drivingLicense)
                                <span class="badge bg-secondary">Not Submitted</span>
                    ...
                    <div class="mobile-card-body">
                        <div class="mobile-field">
                            ...
                            <div class="mobile-field-value">{{ $customer->email }}</div>
                        </div>
                        <div class="mobile-field">
                            <div class="mobile-field-label">Phone</div>
                            <div class="mobile-field-value">{{ $customer->phone }}</div>
                        </div>
                        <div class="mobile-field">
                            <div class="mobile-field-label">CID No.</div>
                            <div class="mobile-field-value">{{ $customer->cid_no }}</div>
                        </div>
                        <div class="mobile-field">
                            <div class="mobile-field-label">License No.</div>
                            <div class="mobile-field-value">{{ $customer->drivingLicense ? $customer->drivingLicense->license_no : 'Not submitted' }}</div>
                        </div>
                        <div class="mobile-field">
                            <div class="mobile-field-label">Registered On</div>
                            <div class="mobile-field-value">{{ \Carbon\Carbon::parse($customer->created_at)->format('d M Y') }}</div>
                        </div>
                    </div>
                    
                    <div class="mobile-card-actions">
                        @if($customer->drivingLicense)
                        <a href="{{ route('admin.user-verification.show', $customer->id) }}" class="btn btn-primary btn-sm w-100">
